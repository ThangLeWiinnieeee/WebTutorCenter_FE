...
// Một dòng đơn nhận lớp trong bảng quản trị, kèm nút duyệt/từ chối và xem chi tiết.
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-4 transition hover:border-slate-300 hover:shadow-sm">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center">
        ...
        <div className="flex items-center gap-3">
          <span className="rounded-lg bg-brand px-2.5 py-1.5 text-xs font-bold text-white shadow-sm">
            #{classItem?.classCode || "—"}
          </span>
          <div className="min-w-0">
            <p className="text-sm font-bold text-slate-800">{classItem?.subject || "—"}</p>
            <p className="text-xs text-slate-400">Ứng tuyển {formatDateTime(application.createdAt)}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {activeTab === "selected" && (
            <Button
              ...
              className="rounded-lg border-brand/30 text-brand hover:bg-brand/5"
            >

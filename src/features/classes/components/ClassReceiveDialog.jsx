...
const DIALOG_CONFIG = {
  ...
  tutorRequired: {
    ...
    description:
      "Tài khoản của bạn chưa có vai trò gia sư. Vui lòng đăng ký và chờ duyệt hồ sơ trước khi nhận lớp.",
  ...
  confirm: {
    ...
    description:
      "Người đăng sẽ xem hồ sơ và chọn gia sư phù hợp. Nếu được chọn, đơn sẽ chuyển admin duyệt và bạn sẽ nhận thông báo.",
  },
  mismatch: {
    icon: AlertTriangle,
    ...
    description:
      "Môn học của lớp này không nằm trong danh sách các môn bạn đăng ký dạy. Vui lòng cập nhật hồ sơ hoặc tìm các lớp học phù hợp hơn.",
  },
  submitted: {
    icon: CheckCircle2,
    iconClassName: "bg-emerald-50 text-emerald-700 border-emerald-100",
    ...
    description:
      "Đơn ứng tuyển của bạn đã được gửi tới người đăng. Nếu được chọn, admin sẽ duyệt lớp — bạn sẽ nhận thông báo khi có kết quả.",
  },
};
...
const ClassReceiveDialog = ({
  ...
  return (
    <Modal
      onClose={onClose}
      labelledBy="class-receive-dialog-title"
      panelClassName="animate-in fade-in-50 zoom-in-95 duration-150"
    >
      <div className="flex items-start justify-between gap-4">
        <div
          className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border ${config.iconClassName}`}
        >
          ...
          {classItem.availabilitySlots && (
            <p>
              ...
              <span className="text-slate-700 font-medium">
                {formatAvailabilitySlotsOneLine(classItem.availabilitySlots)}
              </span>
            </p>
          )}
          <p>

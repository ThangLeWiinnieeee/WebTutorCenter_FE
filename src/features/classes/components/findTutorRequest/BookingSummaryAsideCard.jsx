import { BookOpenCheck, CalendarDays, GraduationCap, MapPinHouse, Users } from "lucide-react";
import { useWatch } from "react-hook-form";

import { formatDate, formatPrice } from "@/features/classes/utils/classFormatters";

// Thẻ tóm tắt bên cạnh form: thông tin lớp đang nhập và học phí tạm tính.
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="space-y-3 text-sm">
        ...
        <p className="flex items-start justify-between gap-3 border-b border-slate-100 pb-2">
          <span className="flex items-center gap-2 text-slate-500">
            <CalendarDays className="h-4 w-4" /> Lịch học
          </span>
          <span className="text-right font-semibold text-slate-800">
        ...
        <p className="flex items-start justify-between gap-3 border-b border-slate-100 pb-2">
          <span className="flex items-center gap-2 text-slate-500">
            <GraduationCap className="h-4 w-4" /> Yêu cầu gia sư
          </span>
          <span className="text-right font-semibold text-slate-800">
            {tutorLevelPref === "teacher"
              ? "Giáo viên"
              : tutorLevelPref === "student"
                ? "Sinh viên"
                : "Không yêu cầu"}
          </span>
        </p>

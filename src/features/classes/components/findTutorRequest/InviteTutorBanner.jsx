...
const InviteTutorBanner = ({ invitedTutor }) => (
  <div className="mb-5 flex items-start gap-3 rounded-2xl border border-brand/20 bg-brand/5 p-4">
    <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-brand" />
    <div className="text-sm text-slate-700">
      <p className="font-semibold text-brand">
        Bạn đang mời gia sư {invitedTutor.fullName}
        {invitedTutor.dateOfBirth ? ` (sinh năm ${new Date(invitedTutor.dateOfBirth).getFullYear()}` : ""}
        {invitedTutor.gender
      ...
      <p className="mt-1 text-slate-600">
        Môn học, khu vực, khung giờ và yêu cầu gia sư đã được giới hạn theo hồ sơ của gia sư này. Lớp sẽ chỉ
        được gửi riêng cho gia sư, không hiển thị công khai.
      </p>
    </div>
  </div>
);

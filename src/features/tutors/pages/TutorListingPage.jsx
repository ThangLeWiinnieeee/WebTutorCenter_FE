...
// Giới tính dùng enum cố định (value là mã lưu trong DB), không lấy từ lookup.
...
// Đọc bộ lọc từ query string trên URL.
...
// Tìm nhãn hiển thị tương ứng với một giá trị trong danh mục.
const labelFor = (list, value) => list.find((x) => String(x.value) === String(value))?.label || value;

// Khung xám giữ chỗ trong lúc đang tải danh sách gia sư.
const CardSkeleton = () => (
...
// Trang danh sách gia sư: lọc, tìm kiếm và phân trang (đồng bộ với URL).
export default function TutorListingPage() {
  ...
  // Nguồn dữ liệu cho các bộ lọc, chọn sao cho khớp với cách lưu trên hồ sơ gia sư.
  ...
  useEffect(() => {
    ...
    locationService
      ...
      .then((res) => {
        if (!cancelled) {
          setDistricts(
            (res.data?.data?.districts || []).map((d) => ({ value: String(d.code), label: d.name })),
          );
        }
      })
  ...
  // Ghi bộ lọc và số trang lên URL để kích hoạt tải lại danh sách.
  ...
  // Áp bộ lọc mới và quay về trang đầu.
  const handleFilterChange = (nextFilters) => applyFilters(nextFilters, 1);

  // Chuyển trang danh sách gia sư.
  ...
  // Gỡ một tiêu chí lọc đang áp dụng.
  ...
  const activeChips = useMemo(() => {
    ...
    if (filters.subject)
      chips.push({ key: "subject", label: `Môn: ${labelFor(lookupsForUI.subjects, filters.subject)}` });
    if (filters.occupationStatus)
      chips.push({
        key: "occupationStatus",
        label: `Chuyên môn: ${labelFor(lookupsForUI.occupations, filters.occupationStatus)}`,
      });
    if (filters.gender)
      chips.push({ key: "gender", label: `Giới tính: ${labelFor(lookupsForUI.genders, filters.gender)}` });
    if (filters.yearOfBirth) chips.push({ key: "yearOfBirth", label: `Năm sinh: ${filters.yearOfBirth}` });
    if (filters.province)
      chips.push({
        key: "province",
        label: `Tỉnh/Thành: ${labelFor(lookupsForUI.provinces, filters.province)}`,
      });
    if (filters.district)
      chips.push({ key: "district", label: `Quận/Huyện: ${labelFor(districts, filters.district)}` });
    return chips;
      ...
      <div className="border-b border-slate-200 bg-linear-to-r from-brand to-[#2c5282]">
        <div className="mx-auto max-w-7xl px-6 py-10" data-aos="fade-down">
          ...
          <p className="mt-2 max-w-2xl text-sm text-slate-200">
            Kết nối với đội ngũ gia sư đã được duyệt hồ sơ — lọc theo môn học, khu vực và kinh nghiệm để tìm
            người phù hợp nhất.
          </p>
        </div>
      </div>

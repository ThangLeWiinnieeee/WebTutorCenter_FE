...
for (const flow of RECEIVING_FLOWS) {
  test(`guide ${flow.id}: steps target real diagram nodes and approval cannot be bypassed`, async () => {
    ...
    const html = await readFile(new URL(`../public/diagrams/class-${flow.id}.html`, import.meta.url), "utf8");
    ...
    assert.ok(html.includes('data-embed'), "Viewer must support embedding");
    if (flow.id !== "cancel") {
      const visits = [];
